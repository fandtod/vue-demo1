<template>
  <div class="page">
        <form class="info">
          評價內容： <input v-model="searchText" type="text" class="age" name="age" />
            價格區間：<input v-model="searchPrice1" type="text" class="age" name="age" />
            ~ &nbsp;&nbsp;&nbsp;&nbsp;<input v-model="searchPrice2" type="text" class="salary" name="salary" />
        ...
        <table>
            ...
            <tbody  @click="stop($event,item.userIdBuyer)" v-for="item in list" :key="item.userIdBuyer">
                <tr>
                    ...
                    <td>{{item.orderServiceItem}}</td>
                    <td>{{item.orderTotal}}</td>
                    <td>{{item.orderPlacementTime}}</td>
                    <td>{{item.orderServiceFinishDate}}</td>
                </tr>
...
export default {
    ...
    data(){
      ...
      getPage(i){
        ...
        axios.get(`http://localhost:8005/homie-demo/searchmember/${this.userId}?page=${this.page}&pageSize=${this.pageSize}&searchText=${this.searchText}&searchPrice1=${this.searchPrice1}&searchPrice2=${this.searchPrice2}`).then(res =>{          
